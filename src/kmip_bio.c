...
int kmip_set_last_result_r(ResponseBatchItem* value, LastResult *result)
{
    if (value && result)
    {
        ...
        result->result_reason = value->result_reason;
        if (value->result_message)
            kmip_copy_textstring(result->result_message, value->result_message, sizeof(result->result_message));
        else
            result->result_message[0] = 0;
    }
    return 0;
}
...
int kmip_bio_get_key_with_context(KMIP *ctx, BIO *bio, char *uuid, int uuid_size, 
    char **key, int *key_size, int *key_type, int *key_algo, LastResult *error)
{
    if(ctx == NULL || bio == NULL || uuid == NULL || uuid_size <= 0 || key == NULL || key_size == NULL)
    {
        return(KMIP_ARG_INVALID);
    ...
    kmip_set_last_result_r(&resp_item, error);
   ...
   CryptoKey *crypto_key = (CryptoKey *)pld->object;
   KeyBlock *block = crypto_key->key_block;
   enum key_format_type format_type;
   switch (pld->object_type)
     {
       case KMIP_OBJTYPE_SYMMETRIC_KEY:
             {
               format_type = KMIP_KEYFORMAT_RAW;
               break;
             }
       case KMIP_OBJTYPE_PUBLIC_KEY:
             {
               format_type = KMIP_KEYFORMAT_PKCS1;
               break;
             }
       case KMIP_OBJTYPE_PRIVATE_KEY:
             {
               format_type = KMIP_KEYFORMAT_PKCS8;
             }
           break;
       default:
           kmip_free_response_message(ctx, &resp_m);
   ...
   if (key_type)
     *key_type = pld->object_type;
   if (key_algo)
     *key_algo = block->cryptographic_algorithm;
    if((block->key_format_type != format_type) ||
       (block->key_wrapping_data != NULL))
    {
        kmip_free_response_message(ctx, &resp_m);
        kmip_set_buffer(ctx, NULL, 0);
