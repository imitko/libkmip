...
/* Example demo AES key meterial */
#define key_data_size 16
unsigned char * key_data = (unsigned char *)"\x78\xBB\x42\xDF\xC5\xA2\x58\x63\x7D\x5A\x71\xE7\xD6\x5E\x31\x1F";
                  ...
                  const char *ca_certificate)
{
    ...
    ByteString key_material = { key_data, key_data_size };
    KeyBlock kb;
    KeyValue kv = {0,0,0};
    kv.key_material = &key_material;
    
    RegisterRequestPayload rrp = {0};
    rrp.object_type = KMIP_OBJTYPE_SYMMETRIC_KEY;
    rrp.template_attribute = &ta;
    kmip_init_key_block(&kb);
    kb.cryptographic_algorithm = KMIP_CRYPTOALG_AES;
    kb.key_format_type = KMIP_KEYFORMAT_RAW;
    kb.cryptographic_length = key_data_size * 8;
    kb.key_value = &kv;
    CryptoKey key = { &kb };
    rrp.object = (void *)&key;
